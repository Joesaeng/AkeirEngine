...
- **Name**: taken from `Game/project.json` → `name` (`TestArena.exe` for the sample; `CatSurvivor.exe` for a project called CatSurvivor). Built only with `AKEIR_WITH_SDL=ON` (`msvc-debug`, `msvc-release`).
