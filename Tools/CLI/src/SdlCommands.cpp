...
bool captureWorld(const PlayWorld& world, int w, int h, const std::string& outPng, std::string* err, Json* stats = nullptr) {
    ...
    if (stats) *stats = Json{{"sprites", s.sprites}, {"texts", s.texts}, {"culled", s.culled}, {"renderMs", s.renderMs}, {"backend", s.backend}, {"camera", s.camera}};
    std::error_code ec;
...
Envelope cmdCapture(Context& ctx) {
    ...
    Json r = Json{{"file", out}, {"width", w}, {"height", h}, {"tick", st.tick}, {"videoDriver", headlessSdl().currentVideoDriver()}, {"renderer", stats.value("backend", "")}, {"sprites", stats.value("sprites", 0)}, {"texts", stats.value("texts", 0)}, {"culled", stats.value("culled", 0)}, {"renderMs", stats.value("renderMs", 0.0)}, {"camera", stats.value("camera", Json())}, {"worldHash", toHex64(world->hash())}};
