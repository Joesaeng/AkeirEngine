...
Json capabilitiesJson() {
    ...
    tools.push_back(tool("play", "Resident play world: open / step / inspect / query / snapshot / close (ADR-0041)",
                         ...
                         props({{"action", Json{{"enum", Json::array({"open", "step", "inspect", "query", "snapshot", "close", "list"})}}}, {"run", Json{{"type", "string"}, {"description", "run id from open"}}}, {"world", S}, {"seed", I},
                                ...
                                {"input", Json{{"type", "object"}, {"description", "{action: value} held on every stepped tick, e.g. {\"MoveX\": 1}; action names from Config/input.json. Optional pointer: {x, y, buttons: [\"left\"], wheel, viewport: [w, h]} (window pixels, default viewport 1280x720). Pressed/released edges are derived from the previous step (ADR-0045)"}}},
    ...
    r["errorCodes"] = Json::array({"USAGE_ERROR", "UNKNOWN_COMMAND", "PROJECT_NOT_FOUND", "CRASH", "TIMEOUT", "CONFIRMATION_REQUIRED", "INTERNAL_ERROR",
                                ...
                                "ASSET_META_INVALID", "ASSET_SOURCE_MISSING", "ASSET_SOURCE_INVALID", "ASSET_SUBASSET_RECT_INVALID", "ASSET_SUBASSET_MISSING", "EXPR_PARSE_ERROR", "PHYSICS_LAYERS_INVALID", "PHYSICS_LAYER_UNKNOWN", "RUN_REQUIRES_SERVE", "ASSET_KIND_MISMATCH"});
    return r;
}
