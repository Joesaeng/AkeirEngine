...
Json callTool(ServeHost& host, const std::string& name, const Json& a) {
    ...
    if (name == "capabilities") { argv = {"capabilities"}; }
    ...
    else if (name == "run") { argv = {"run", "--headless"}; opt("ticks", num(a, "ticks")); opt("seed", num(a, "seed")); opt("world", str(a, "world")); opt("hash-every", num(a, "hashEvery")); opt("snapshot-out", str(a, "snapshotOut")); boolFlag("profile", "profile"); }
        ...
        else if (action == "step") { argv = {"run", "step", str(a, "run")}; opt("ticks", num(a, "ticks")); if (a.contains("input") && a["input"].is_object()) { argv.push_back("--input"); argv.push_back(a["input"].dump()); } boolFlag("profile", "profile"); }
