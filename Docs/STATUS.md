...
빌드: `scriptsuild.cmd msvc-headless all` 과 `msvc-release all`(SDL3 포함) 통과. 테스트: `akeir_tests.exe` 99 케이스(msvc-release; headless 는 94) 통과 + `akeir test` 4 시나리오(SDL) / 3 + 1 skipped(headless) 통과 (2026-08-22). 샘플 `Game/` 은 `validate` 0 error / 0 warning(canonical), `run --headless --ticks 600` 의 기준 finalHash = `0xf9a1151e01338658` (v0.1.3: SpriteRenderer/TextRenderer 에 reflected 멤버가 늘고 HUD bar entity 2개가 추가되어 바뀜 — component 의 모든 reflected 값이 해시에 들어간다; 불변 fixture `Tests/Fixtures/TestArena` 도 같은 이유로 `0x1fc2a036cee79712`. 이 값이 바뀌면 결정론, 샘플 데이터, 또는 component schema 가 바뀐 것).
git: 공개 저장소 **https://github.com/Joesaeng/AkeirEngine** (branch `main`, 태그 `v0.1.0` → `v0.1.1` → `v0.1.2` → **`v0.1.3`**(2026-08-22: CatSurvivor 2차 피드백 ADR-0042~0046; 실험용 zip 은 이것), GitHub Release 에 `AKEIR-<ver>.zip` 첨부 — 소스 + Docs + `bin/akeir.exe` + QUICKSTART; 리서치 자료와 `.pdb` 는 제외). 릴리즈 zip 은 `python scripts/package.py`. CI: `.github/workflows/ci.yml` (headless + SDL job) — 2026-08-22 기준 **green**, runner(MSVC 14.51) 의 finalHash 도 기준값과 동일. 태그 `v0.1.0` 은 CI 가 green 이 된 커밋(6ed8d4f) 으로 옮기고 zip 을 재업로드했다. **커밋/푸시는 사용자가 지시할 때만.**
...
- PlayWorld 의 query 는 선형 스캔(이름은 호출당 1회 해석, ADR-0044): Stress world(100 goblin, 107 query/tick) 0.33 ms/tick. Flecs 쿼리 파이프라인/spatial query 는 profile 수치가 요구할 때.
- body 가 있는 entity 의 Transform.x/y 는 physics 가 소유한다 (ADR-0016) — system 이 Transform.position 을 직접 바꿔도 body 에는 반영되지 않는다.
- `Collider2D.layer` → Box2D filter 는 `project.json physics.layers` 로 (ADR-0043). 선언이 없으면 전부 충돌.
...
- **2026-08-22 (v0.1.3 릴리즈)**: 버전 0.1.3, `AKEIR-0.1.3.zip`. 내용 = CatSurvivor 2차 피드백 A~D(ADR-0042~0046) 전부. 코덱스에게 다시 맡겨 3차 피드백을 받는 것이 다음.
- **2026-08-22 (CatSurvivor v0.1.2 피드백, D)**: ADR-0046 — Font asset(`importer: Font`, `akeir asset import X.ttf`) + `TextRenderer.font/size`, stb_truetype(CPM, commit 고정) 글리프 atlas, 한글 확인(시스템 malgun.ttf 로 테스트·캡처). v0.1.3 릴리즈.
- **2026-08-22 (CatSurvivor v0.1.2 피드백, C)**: ADR-0045 — pointer 입력(InputFrame.pointer, mouse 바인딩, 포커스 release) + action/pointer edge 를 `withEdges` 한 곳에서(test DSL `pointer` step, play step, replay), `SpriteRenderer.screenSpace/anchor/pixelSize/fill` + `akeir/ecs/Screen.h`(renderer 와 같은 rect 로 hitTest), `PlayWorld::setPaused` + `runWhilePaused`. 샘플에 HP bar·P pause. SpriteRenderer 스키마 변화로 기준 finalHash `0xf9a1151e01338658`(HUD bar entity 2개 포함). 다음: D(TTF font asset) → v0.1.3.
- **2026-08-22 (CatSurvivor v0.1.2 피드백, A/B)**: 코덱스 2차 피드백(`CATSURVIVOR_V012_FEEDBACK.md`). 실측으로 "1분 후 버벅임" 은 Debug 빌드(Release 0.2–0.7 ms/tick vs Debug 15배) — ADR-0042(stale contact event 버그 + entityForBody 역인덱스 + `<Name>-debug.exe`/buildConfig), ADR-0043(collision matrix), ADR-0044(profiler·culling·query 4.3× ·Stress world·perf 스크립트 CI). 다음: C(pointer input, screen-space sprite, pause) → D(TTF font asset) → v0.1.3.
