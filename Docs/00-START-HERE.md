...
- **이름**: MoltEngine (ME). 저장소 디렉터리는 `Project_ME`, C++ 네임스페이스·CMake 타깃 접두어는 `pme`(Project ME). 첫 릴리즈 = **ME0.1** (git tag `ME0.1`, `v0.1.0`; `game version` 의 `release`).
...
build\msvc-headless\bin\game.exe capabilities --json
build\msvc-headless\bin\game.exe debug crash-test        # exit 6 + Cache/crash/*.dmp
build\msvc-headless\bin\game.exe debug hang-test --timeout 2s   # exit 7
cd Game && ..\build\msvc-headless\bin\game.exe run --headless --ticks 600 --json   # 결정론 실행 (finalHash)
cd Game && ..\build\msvc-headless\bin\game.exe set name:Goblin_01 Health.max 45 --dry-run --json   # 쓰기 명령 (dry-run)
cd Game && ..\build\msvc-headless\bin\game.exe test --json                                        # 데이터화 테스트 (§23)
cd Game && ..\build\msvc-debug\bin\game.exe run --json                                              # 창 모드 (SDL 빌드)
cd Game && ..\build\msvc-debug\bin\game.exe capture --ticks 300 --out Cache\capture\f.png --json      # PNG capture (§27)
cd Game && ..\build\msvc-headless\bin\game.exe serve                                                # 상주 프로세스 (다른 창의 game 명령이 포워딩됨)
cd Game && ..\build\msvc-headless\bin\game.exe mcp                                                  # MCP 서버 (stdio)
```

## 7. 핵심 불변 조건 (깨지면 설계가 깨진 것)

- authoring JSON(`Game/`)이 source of truth. Flecs/Box2D 는 투영이다 (§88.2).
- 파일을 바꾸는 경로는 `CommandBus`(ChangeSet 을 남김)와 `game fmt`(canonical 재직렬화)뿐이다.
- 모든 ChangeSet 은 self-inverting 이다 — undo 뒤 파일은 byte-identical 이어야 한다 (Tests/Commands_Bus.cpp 가 확인).
- `game run --headless` 는 같은 입력에 같은 `finalHash` 를 낸다 (T0). `game test` 의 run-twice 가 이것을 시나리오마다 확인한다.
- 출력은 항상 §12 envelope, 오류는 §79 Diagnostic 형태 + `ruleId`.
- 골든 이미지는 software renderer(`game capture` / `game test --update-golden`)로만 만든다 — 창(GPU) 픽셀은 비교에 쓰지 않는다 (ADR-0026).
- 프로젝트의 writer 는 하나다: `game serve` 가 떠 있으면 그것, 아니면 각 one-shot 명령. CLI·MCP·(Editor) 는 전부 같은 `CommandSpec` 표와 `ServeHost` 를 쓴다.
