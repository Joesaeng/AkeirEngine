...
set(AKEIR_DEP_CACHE "${CMAKE_SOURCE_DIR}/.cpm-cache")

# akeir_add_dep(NAME <cpm name> DIR <cache dir> REPO <owner/repo> TAG <tag> OPTIONS <opt>...)
...
macro(akeir_add_dep)
  ...
  if(EXISTS "${AKEIR_DEP_CACHE}/${D_DIR}/CMakeLists.txt")
    message(STATUS "deps: ${D_NAME} ${D_TAG} <- .cpm-cache/${D_DIR}")
    CPMAddPackage(NAME ${D_NAME} SOURCE_DIR "${AKEIR_DEP_CACHE}/${D_DIR}" OPTIONS ${D_OPTIONS})
  else()
...
# ---- 버전 고정 표 (§3) ----
set(AKEIR_DEP_FLECS_TAG    v4.1.6)         # MIT, C99 core + C++17 API  (2026-06-29)
set(AKEIR_DEP_BOX2D_TAG    v3.1.1)         # MIT, C17                    (2025-06-04)
...
# ---- Flecs (ECS, §3.1 기본 후보) ----
akeir_add_dep(NAME flecs DIR flecs REPO SanderMertens/flecs TAG ${AKEIR_DEP_FLECS_TAG}
  OPTIONS "FLECS_STATIC ON" "FLECS_SHARED OFF" "FLECS_PIC OFF" "FLECS_TESTS OFF")

# ---- Box2D v3 (2D physics, §57) ----
akeir_add_dep(NAME box2d DIR box2d REPO erincatto/box2d TAG ${AKEIR_DEP_BOX2D_TAG}
  OPTIONS "BOX2D_SAMPLES OFF" "BOX2D_UNIT_TESTS OFF" "BOX2D_BENCHMARKS OFF" "BOX2D_VALIDATE OFF")

# ---- doctest (unit tests) ----
if(AKEIR_BUILD_TESTS)
  akeir_add_dep(NAME doctest DIR doctest REPO doctest/doctest TAG ${AKEIR_DEP_DOCTEST_TAG}
    OPTIONS "DOCTEST_WITH_TESTS OFF" "DOCTEST_NO_INSTALL ON")
endif()

# ---- SDL3 (platform / window / input / headless drivers, §20) ----
option(AKEIR_WITH_SDL "Build with SDL3 (window/input). OFF = headless-only build" ON)
if(AKEIR_WITH_SDL)
