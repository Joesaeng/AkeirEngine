...
| §88.3 | `Config/input.json` 의 gamepad 바인딩은 파싱만 하고 `unsupported` 로 보고. mouse 바인딩과 pointer 는 ADR-0045 로 구현(아래). |
| §88.3, §23 (ADR-0045) | `InputFrame` 에 `pointer{x,y,viewport,buttons,pressed,released,wheel,inside}` 와 action edge(`pressedActions/releasedActions`, `justPressed()`) — edge 는 `InputFrame::withEdges` 한 곳에서(InputMap·test DSL `pointer` step·`play step`·replay 공통). 포커스 잃음 = 전부 release. |
| §27, §19 (ADR-0045) | `SpriteRenderer.screenSpace/anchor/pixelSize/fill`; `akeir/ecs/Screen.h` 의 `spriteScreenRect/hitTest/worldToScreen` 을 렌더러와 게임이 같이 쓴다. `PlayWorld::setPaused` + `addSystem(..., runWhilePaused)`. |
| §27, §37 (ADR-0046) | `importer: Font` sidecar(`akeir asset import X.ttf`) + `TextRenderer.font/size` — stb_truetype 글리프 atlas(한글 등 폰트가 가진 모든 유니코드), 빈 font 는 5x7 bitmap. |
| §57 (ADR-0042, ADR-0043) | contact 중 despawn 된 body 의 stale End 이벤트는 Box2D 층에서 버린다. `PlayWorld::entityForBody(handle)`. `project.json physics.layers` → category/mask(대칭, sensor 포함), `PHYSICS_LAYER_UNKNOWN`. |
| §16, §27, §72 (ADR-0044) | `PlayWorld::profile()`(system/physics/query ms, `run --profile`, `play step {profile}`), Renderer2D frustum cull + `renderMs`, query 이름 1회 해석(Stress 100 goblin 0.33 ms/tick), `Game/Worlds/Stress.world.json` + `scripts/test_perf.py`. Debug 게임 exe 는 `<Name>-debug.exe`. |
