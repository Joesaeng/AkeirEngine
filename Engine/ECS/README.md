...
- `setPaused(bool)` + `addSystem(name, fn, phase, runWhilePaused)` — paused ticks run only the flagged systems and skip physics; tick numbers keep counting (ADR-0045).
- `akeir/ecs/Screen.h` — `primaryCamera`, `worldToScreen/screenToWorld`, `spriteScreenRect`, `hitTest(world, viewport, px)`: the one rect definition shared by Renderer2D and game-side pointer hit tests (ADR-0045).
