// akeir/reflection/Reflect.h — 등록 front-end (매크로 + builder). 설계 문서 §42.2.
//
...
//       AKEIR_PROP(max, "Maximum hit points").min(1).def(100).unit("hp").ui(1, 1000, 1);
//       AKEIR_PROP(current, "Current hit points").runtimeOnly().readOnly();
//   AKEIR_REFLECT_END(Health)
//
//   enum 은 선언 뒤 전역 네임스페이스에서 이름을 등록한다 (값 = 선언 순서 index):
//       enum class ColliderShape { Box, Sphere, Capsule };
//       AKEIR_REFLECT_ENUM(ColliderShape, "box", "sphere", "capsule")
//
...
#define AKEIR_REFLECT_ENUM(EnumType, ...)                                                                 \
    template <> struct akeir::EnumTraits<EnumType> {                                                      \
        static constexpr bool kDefined = true;                                                          \
        static const std::vector<std::string>& names() {                                                \
            static const std::vector<std::string> n = ::akeir::detail::splitNames(#__VA_ARGS__);          \
            return n;                                                                                   \
        }                                                                                               \
    };
...
template <class E> struct PropTraits<E, std::enable_if_t<std::is_enum_v<E>>> {
    static_assert(EnumTraits<E>::kDefined, "enum must be registered with AKEIR_REFLECT_ENUM(EnumType, \"a\", \"b\", ...)");
...
#define AKEIR_REFLECT_BEGIN(Type, Description)                                                   \
    namespace {                                                                                \
    struct AkeirReflect_##Type {                                                                 \
        AkeirReflect_##Type() {                                                                  \
            using AkeirSelf_ = Type;                                                             \
            ::akeir::ComponentBuilder<Type> b_(#Type, Description);

#define AKEIR_REQUIRES(Name) b_.requiresComponent(Name)
...
#define AKEIR_REFLECT_END(Type)                                                                  \
    ...
    static AkeirReflect_##Type akeirReflectInstance_##Type;                                        \
    }
