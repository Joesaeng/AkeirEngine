# Engine/Reflection (`pme_reflection`)
...
| 헤더 | § | 제공 |
|---|---|---|
| `pme/reflection/PropertyMeta.h` | §42.2, §43.1, §88.8 | `PropFlags`, `PropType`, `Visibility` + `isVisible()`(유일한 정의), `PropertyMeta`(get/set 접근자, `toSchema`), `ComponentMeta`(`toSchema`, `toWireFormat`, `defaultJson`, ctor/dtor/copy/move 훅) |
| `pme/reflection/Registry.h` | §42.2, §14.1 | `Registry::global()` (이름/typeid 조회, 이름순 `all()`), `getByPointer` / `setByPointer` (`/position/0`), `splitPropertyPointer`, `allSchemas` |
| `pme/reflection/Reflect.h` | §42.2 | `PME_REFLECT_BEGIN/PROP/REQUIRES/LIFECYCLE/VERSION/END`, `PME_REFLECT_ENUM`, `PropTraits<T>` (bool/int/float/double/string/enum/Vec2-4/Quat/Color/Ref), `ComponentBuilder`, `PropBuilder` |
...
PME_REFLECT_END(Health)          // 타입 이름을 다시 준다 (토큰 결합)

enum class Shape { Box, Circle };
PME_REFLECT_ENUM(Shape, "box", "circle")   // 전역 네임스페이스에서. 값 = 선언 순서 index, JSON 은 문자열
```

## 규칙 / 함정
- component 는 **aggregate** (`static_assert`). 이름은 PascalCase, property 는 camelCase. 중첩 struct 는 안 됨 — Vec/Quat/Color/Ref 가 leaf.
- `PME_REFLECT_BEGIN(Type, …)` 은 **unqualified 이름**으로, 그 타입의 네임스페이스 안에서 쓴다 (`namespace game { PME_REFLECT_BEGIN(Health, …) … }`). `PME_REFLECT_ENUM` 은 전역에서 qualified 이름으로.
