...
std::uint64_t fnv1a64(const void* data, std::size_t size, std::uint64_t h) {
    ...
    for (std::size_t i = 0; i < size; ++i) { h ^= p[i]; h *= kFnvPrime64; }
    return h;
}
...
std::string toHex(const std::uint8_t* data, std::size_t size) {
    ...
    std::string out;
    out.reserve(size * 2);
    for (std::size_t i = 0; i < size; ++i) { out.push_back(digits[data[i] >> 4]); out.push_back(digits[data[i] & 0x0F]); }
    return out;
}
...
std::array<std::uint8_t, 32> Sha256::finish() {
    ...
    std::uint8_t len[8];
    for (int i = 0; i < 8; ++i) len[i] = static_cast<std::uint8_t>(bits >> (8 * (7 - i)));
    update(len, 8);
...
std::string Sha256::hexOfBytes(const void* data, std::size_t size, bool withPrefix) {
    Sha256 s;
    s.update(data, size);
    auto d = s.finish();
