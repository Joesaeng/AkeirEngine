// akeir/core/Id.h — Persistent ID. 설계 문서 §7.1 (TypeID v0.3 grammar / RFC 9562 UUIDv7·v8).
//
...
template <> struct std::hash<akeir::Id> {
    std::size_t operator()(const akeir::Id& id) const noexcept { return std::hash<std::string>{}(id.str()); }
};
