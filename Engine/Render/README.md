...
- `TextRenderer.font` (ADR-0046): `src/FontCache.{h,cpp}` loads the TTF/OTF with stb_truetype, rasterizes glyphs on first use into shelf-packed atlas pages per (asset, pixel height) and draws them with the text color as color mod; kerning, left/center/right alignment, Unicode (Korean). Empty or unresolvable `font` → the built-in 5x7 bitmap font. `RenderStats.glyphs`.
