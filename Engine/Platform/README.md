...
| `include/pme/platform/Interactive.h`, `src/Interactive.cpp` | `runInteractive(platform, renderer, world, input, cfg)` — wall clock accumulator 로 fixed tick 을 몇 번 돌릴지 정하고 매 프레임 `renderer.render/present`. `maxFrameSeconds`(0.25) 로 spiral-of-death 방지. `recordInputsPath` 면 tick 마다 `InputFrame.toJson()` 한 줄(JSONL) — `game run --headless --replay` 입력 형식과 같다 |
