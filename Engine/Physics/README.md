...
결정론: Box2D 는 worker 없이(단일 스레드) 돈다. v3.1+ 은 worker 수와 무관하게 bit-exact (FAQ). MSVC `/fp:precise` 는 `PME_FP_FLAGS_OPTIONS` 로 Box2D 타깃에도 걸려 있다 (cmake/Dependencies.cmake). Box2D 에는 SaveState 가 없으므로 snapshot 복원 = world 재생성 (§26.1, §58).
